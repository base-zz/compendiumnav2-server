...
async function initializeServices() {
  try {
    ...
    console.log("[DEV-SERVER2] Initializing TidalService and WeatherService...");
    ...
    console.log("[DEV-SERVER2] Initial state structure:", JSON.stringify({
      navigation: stateService.state.navigation,
      environment: stateService.state.environment,
      vessel: {
        info: stateService.state.vessel.info,
        systems: { 
          electrical: Object.keys(stateService.state.vessel.systems.electrical),
          tanks: Object.keys(stateService.state.vessel.systems.tanks || {})
        }
      }
    }, null, 2));
...
async function bridgeStateToRelay() {
  ...
  try {
    ...
    console.log("[DEV-SERVER2] Sending initial state to relay:", JSON.stringify({
      position: currentState.navigation.position,
      tanks: currentState.vessel.systems.tanks,
      fuelTanks: {
        fuel1: currentState.vessel.systems.propulsion.fuel1,
        fuel2: currentState.vessel.systems.propulsion.fuel2
      },
      batteries: Object.fromEntries(
        Object.entries(currentState.vessel.systems.electrical)
...
async function startDevServer() {
  try {
    ...
    console.log(`[DEV-SERVER2] Starting WebSocket server on port ${directPort}`);
    ...
    console.log(`[DEV-SERVER2] Starting relay server on port ${relayPortFinal}`);
    ...
    if (relayServer && relayServer.vpsConnector) {
      relayServer.vpsConnector.on('connected', () => {
        console.log(`[DEV-SERVER2] Successfully connected to VPS at ${relayConfig.vpsUrl}`);
      });
    ...
    const shutdown = async () => {
      ...
      try {
        ...
        setTimeout(() => {
          console.error('[DEV-SERVER2] Could not close connections in time, forcefully shutting down');
          process.exit(1);
        }, 10000);
      } catch (error) {
