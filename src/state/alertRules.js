/**
 * Alert Rules for the state manager
 * These rules define when alerts should be triggered and resolved
 */
...
export const AlertRules = [
  ...
  // AIS Proximity Resolution Rule
  {
    ...
    condition: (state) => {
      ...
      // Check if there are no more vessels in warning range
      const anchorState = state.anchor || {};
      const aisTargets = state.ais?.targets || [];
      ...
      // Count vessels in warning range
      const targetsInRange = aisTargets.filter(target => {
        // Calculate distance between target and anchor
        const distance = calculateDistance(
          target.position.latitude,
      ...
      // Rule triggers when no vessels are in warning range
